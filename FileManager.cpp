...
void FileManager::modifyStock(string fileName, string content, int modifiedStock)
{
	int contentLineNum = findContentLine(fileName, content); //바꾸기 원하는 content의 lineNum을 contentLineNum에 저장
	int fileLineNum = countLines(fileName); //file의 lineNum
	int i;
	...
	string tempLine; //파일을 tempLine에 한줄씩 담아 writeLine에 옮김
	size_t pos;
	targetLine[1] = to_string(modifiedStock);
	open(fileName);
	if (contentLineNum == -1)
	{
		close();
...
void FileManager::update_for_here_to_go(string s) { // 매장
	ofstream writeFile("sold_type.txt", ios::trunc);
	writeFile << s;
	writeFile.close();
}
